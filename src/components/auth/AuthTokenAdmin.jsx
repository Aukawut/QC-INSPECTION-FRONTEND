...
function AuthTokenAdmin() {
  ...
  const authen = useCallback(async () => {
    ...
    await axios
      ...
      .then((res) => {
        ...
        if (
          res.data.info.decoded.ROLE === 'Admin' ||
          res.data.info.decoded.ROLE === 'Super Admin'
        ) {
  ...
  return (
    <div>
      <p className="hidden">Auth Admin</p>
    </div>
  );
}

export default AuthTokenAdmin;
