...
const EditInspection = ({ psthPartNo ,obj}) => {
  const baseURL = import.meta.env.VITE_BASE_URL;
  console.log("_________",psthPartNo);
    ...
    await axios
      ...
      .then((res) => {
        if (res.data.err) {
          ...
          getDataXBarChart(res.data.result[0].MOLD_NO);
        }
      })
      .catch((err) => {
        console.log(err);
    ...
    <div ref={containerRef} style={{ overflowY: 'auto', maxHeight: 'calc(100vh - 100px)' }}>
      ...
      <Grid container>
        <Grid item xs={12} lg={7}>
          ...
          <Box className="mt-[1rem] mb-1">
            <FormControl className="w-[200px]">
              ...
              <Select
                ...
                size="small"
                onChange={(e) => {
                  getDataXBarChart(e.target.value);
          ...
          {standardPart.map((val, index) => (
              <>
                <Label color="primary">Point No. {index + 1}</Label>
                {!isManual ? (
                  <div>
                ...
                <Typography fontSize={12} marginTop={1}>X Bar Chart - {index + 1}</Typography>
                <XBarChart
                  key={index}
                  isLoad={false}
                ...
                <Typography fontSize={12} marginTop={1}>R Bar Chart - {index + 1}</Typography>
                <RBarChart
                  key={index}
                  isLoad={false}
                  lslValue={parseFloat(val?.LSL)}
            ...
            <div className="flex ml-[5.5rem] gap-x-2 mt-3">
              <div className="flex flex-col justify-center text-center">
                ...
                <TextField
                  ...
                  onChange={(e) => setCavitySimple1(e.target.value)}
                />
              </div>
              <div className="flex flex-col justify-center text-center">
                <p>Simple 2</p>
                <TextField
                  ...
                  value={cavitySimple2}
                  onChange={(e) => setCavitySimple2(e.target.value)}
                  onKeyPress={(e) => {
                    if (e.key === 'Enter') {
