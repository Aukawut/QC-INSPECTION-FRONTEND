...
import ModalControlChart from '../modal-control-chart';

const InspectionPartView = () => {
  const baseURL = import.meta.env.VITE_BASE_URL;
  const psthPartNo = useParams();
  ...
  const [cavitySimple1, setCavitySimple1] = useState('');
  const [cavitySimple2, setCavitySimple2] = useState('');
    ...
    await axios
      ...
      .then((res) => {
        if (res.data.err) {
          ...
          setDateControlChart(new Date().toJSON().slice(0, 10));
          getXBarControlLine(100, res.data.result[0].MOLD_NO, new Date().toJSON().slice(0, 10));
          getDataXBarChart(res.data.result[0].MOLD_NO, new Date().toJSON().slice(0, 10));
        }
      })
      .catch((err) => {
        console.log(err);
      });
  }, [baseURL, psthPartNo.partNo, getXBarControlLine, getDataXBarChart]);
  const handleMoldNoChange = (e) => {
    if (
      e.target.value === '' ||
      (e.target.value > 0 &&
        e.target.value <= 10 &&
      ...
      setMoldNo(e.target.value);
      setMold(e.target.value);
    }
    // เมือ Mold No มีการเปลี่ยนแปลงถ้า ไม่ใช่ค่าว่างเรียกข้อมูลการ Inspection By Mold
    if (e.target.value !== '' && e.target.value !== '0' && e.target.value !== 0) {
      getDataXBarChart(e.target.value, new Date().toJSON().slice(0, 10));
      getXBarControlLine(topData, e.target.value, new Date().toJSON().slice(0, 10));
    }
  };
  useEffect(() => {
    console.log('Effefct');
          ...
          <Box fontSize={12} marginTop={3}>
            ...
            <Box className="flex items-center mb-[1rem]">
              ...
              <TextField
                ...
                type="text"
                value={moldNo}
                onChange={handleMoldNoChange}
                onKeyPress={(e) => {
                  if (e.key === 'Enter') {
                    if (e.target.value !== '') {
                      inputRefInspector.current.focus();
                    }
                  }
                }}
              />
            ...
            <Box className="flex items-center mb-[0.2rem]">
              ...
              <TextField
                size="small"
                placeholder="Enter username such as AWK"
                id="outlined-basic"
                inputProps={{
                  style:{
                    fontSize:15
            ...
            <span className="text-[red] text-[12px] ml-[5rem]">* Enter your username such as awk</span>
            <Box className="mt-[1rem]">
              {time === '' ? (
                <Typography fontSize={12} fontWeight="bold" marginRight={2} component="span">
                  Time : <span className="text-[red]">* Not select time</span>
                </Typography>
              ) : (
                <Typography fontSize={12} fontWeight="bold" marginRight={2} component="span">
                  Time : {time} น.
...
export default InspectionPartView;
